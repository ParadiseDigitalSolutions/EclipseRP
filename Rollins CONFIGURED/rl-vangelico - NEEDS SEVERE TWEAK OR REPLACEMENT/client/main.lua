    ...
    if QBCore ~= nil and isLoggedIn then
        ...
        if #(pos - vector3(Config.JewelLocation["ThermiteSecurity"].x, Config.JewelLocation["ThermiteSecurity"].y,Config.JewelLocation["ThermiteSecurity"].z)) < 1.0 then
            if CurrentCops >= Config.RequiredCops then
                QBCore.Functions.TriggerCallback("rl-jewellery:Callback:Cooldown",function(isCooldown)
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                            if hasItem then
                                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                TriggerServerEvent("rl-jewellery:server:SetHackSecurityStatus", "isBusy", true)
                                ...
                                function() -- Successfully Disable Cameras
                                    ThermiteAnimation() 
                                    ThermiteSuccess()
                                    TriggerServerEvent('rl-jewellery:BeginCooldown')
    ...
    if #(pos - vector3(Config.JewelLocation["DisableCameras"].x, Config.JewelLocation["DisableCameras"].y,Config.JewelLocation["DisableCameras"].z)) < 1.5 then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if Config.JewelLocation["ThermiteSecurity"].isDone then
                if hasItem then
                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                    TriggerServerEvent("rl-jewellery:server:SetThermiteSecurityStatus", "isBusy", true)
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("rl-jewellery:server:SetJewelLocations")
end)

RegisterNetEvent('rl-jewellery:client:ConfigLocs')
AddEventHandler('rl-jewellery:client:ConfigLocs', function(list)
    Config.JewelLocation = list
end)

-- Hacking Security System!
RegisterNetEvent('rl-jewellery:client:SetThermiteSecurityStatus')
AddEventHandler('rl-jewellery:client:SetThermiteSecurityStatus', function(stateType, state)
...
RegisterNetEvent('rl-jewellery:client:SetCameraStatus')
AddEventHandler('rl-jewellery:client:SetCameraStatus', function(stateType, state)
    if stateType == "isBusy" then
...
function ThermiteFailed()
    ...
    TriggerServerEvent("rl-jewellery:server:SetThermiteSecurityStatus", "isBusy", false)    
end
...
function ThermiteSuccess()
    ...
    TriggerServerEvent('rl-jewellery:Server:BeginCooldown')
    ...
    if #(pos - vector3(Config.JewelLocation["ThermiteSecurity"].x, Config.JewelLocation["ThermiteSecurity"].y,Config.JewelLocation["ThermiteSecurity"].z)) < 1.5 then
        TriggerServerEvent("rl-jewellery:server:SetThermiteSecurityStatus", "isDone", true)
        TriggerServerEvent("rl-jewellery:server:SetThermiteSecurityStatus", "isBusy", false)  
    end
end
...
function SecurityFailed()
    ...
    PlaySound(-1, "Place_Prop_Fail", "DLC_Dmod_Prop_Editor_Sounds", 0, 0, 1)
    TriggerServerEvent("rl-jewellery:server:SetCameraStatus", "isBusy", false)    
end
...
function SecuritySuccess()
    ...
    local pos = GetEntityCoords(PlayerPedId())
    TriggerServerEvent("QBCore:Server:RemoveItem", "sec_d", 1)
    if #(pos - vector3(Config.JewelLocation["DisableCameras"].x, Config.JewelLocation["DisableCameras"].y,Config.JewelLocation["DisableCameras"].z)) < 1.5 then
        TriggerServerEvent("rl-jewellery:server:SetCameraStatus", "isDone", true)
        TriggerServerEvent("rl-jewellery:server:SetCameraStatus", "isBusy", false)  
    end
end
...
RegisterNetEvent('rl-jewellery:client:setVitrineState', function(stateType, state, k)
